...
import { Shipment, ShipmentList, DateRange } from 'src/commonDS/DS';
...
export class DisplayShipmentListComponent implements OnInit , OnDestroy{
  ...
  selected: boolean[] = [];
  isAllAwbSelected:boolean = false;
  dateRange!: DateRange;
  ...
  constructor(private data: DataService, private crudOperation: CrudService) { 
    ...
    this.subscription = this.data.currentDateRange.subscribe((dRange:DateRange) => {this.dateRange = dRange});
  }
  ...
  onDeleteAWB() {
    if(this.isAllAwbSelected) {
      return(this.onAllAWBDelete());
    }
  ...
  onAllAWBDelete() {
    ...
    this.crudOperation.onAllDeleteShipment(this.dateRange.startDate, this.dateRange.endDate).subscribe((rsp) => {
      alert("Shipment(s) is/are Delete successfully.");
    },
    (error) => {
      alert("Shipment deletion failed");
